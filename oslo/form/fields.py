...
class Field(object):
    ...
    def valid(self, handler):
        ...
        :return:
        """
        raise NotImplementedError('your class %s must implement valid method' %
                                  self.__class__)
...
class StringField(Field):
    ...
    def valid(self, handler):
        ...
        input_value = "%s" % handler.request_body().get(self.name, "")
        ...
        if self.key:
            ret = re.match(self.REGULAR_KEY, input_value)
            if not ret:
        ...
        if self.max_length:
            if len(input_value) > self.max_length:
                if self.custom_error_dict.get('max_length', None):
                    ...
                    self.error = "%s max length is %s" % (self.name,
                                                          self.max_length)
                return
        ...
        if self.min_length:
            if len(input_value) < self.min_length:
                if self.custom_error_dict.get('min_length', None):
                    ...
                    self.error = "%s min length is %s" % (self.name,
                                                          self.min_length)
                return
        self.status = True
...
class IntegerField(Field):
    ...
    def valid(self, handler):
        ...
        """
        # input_value = FrameworkFactory.get_framework().get_argument(handler, self.name, None)
        input_value = "%s" % handler.request_body().get(self.name, None)
...
class StringListField(Field):
    ...
    def valid(self, handler):
        ...
        input_value = handler.request_body().get(self.name, "")

        LOG.debug("input_value: [%s]", input_value)
        self.value = input_value
            ...
            if self.ele_max_length:
                if len(value) > self.ele_max_length:
                    if self.custom_error_dict.get('ele_max_length', None):
                        ...
                        self.error = "element %s max length is %s" % (
                            self.name, self.ele_max_length)
                    return
            ...
            if self.ele_min_length:
                ...
                if len(value) < self.ele_min_length:
                    if self.custom_error_dict.get('ele_min_length', None):
                        ...
                        self.error = "element %s min length is %s" % (
                            self.name, self.ele_min_length)
                    return
...
class BoolField(Field):
    """
        bool值类字段  1 = ture 0 = false
    """

    REFALSE = (False, "false", "False", 0, "0")
    RETRUE = (True, "true", "True", 1, "1")

    def __init__(self, default=None, error=None, required=True):
        """
        :param error: 自定义错误信息
                      如 {
                          'required': '值为空的错误提示',
        ...
        return super(BoolField, self).__init__()

    def valid(self, handler):
        """
        从请求中获取用户输入的值并和规则进行比较
        :param handler: tornado处理请求的hanlder对象
        :return: 
        """
